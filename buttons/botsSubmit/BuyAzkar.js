...
const tokens = new Database("/tokens/tokens.json")
const tier1subscriptions = new Database("/database/makers/tier1/subscriptions")
const AzkarDB = new Database("/Json-db/Bots/AzkarDB.json")
const fs = require('fs');

let Broadcast = tokens.get(`Azkar`)
        ...
        if(interaction.customId == "BuyAzkar_Modal") {
            ...
            try{
              const owner = interaction.user.id
                let price1 = prices.get(`Azkar_price_${interaction.guild.id}`) || 15;
            ...
            require("../../Bots/azkar/handlers/events")(client14)
            require("../../events/requireBots/Azkar-commands")(client14);
            const folderPath = path.resolve(__dirname, '../../Bots/azkar/slashcommand14');
            ...
            for (let file of readdirSync(folderPath2).filter(f => f.endsWith('.js'))) {
                ...
                client14.on("interactionCreate" , async(interaction) => {
                  ...
                  if(!Broadcast) {
                      ...
                      await tokens.add(`Azkar` , [{token:Bot_token,prefix:Bot_prefix,clientId:client14.user.id,owner:interaction.user.id,timeleft:2629744}])
                      
                  }else {
                      await tokens.push(`Azkar` , {token:Bot_token,prefix:Bot_prefix,clientId:client14.user.id,owner:interaction.user.id,timeleft:2629744})
