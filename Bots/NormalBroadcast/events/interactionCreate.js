...
*/const { Events, Interaction, EmbedBuilder ,InteractionType } = require('discord.js');
const { Database } = require("st.db")
const Broadcast = require(`../../../tokens/tokens.json`)
const client18 = require(`../Broadcast-Bots.js`)
module.exports = {
    ...
    if (interaction.isChatInputCommand()) {
      console.log('tst2')
	    if(interaction.user.bot) return;
	     let client = interaction.client18;
		const command = interaction.client18.Broadcast2SlashCommands.get(interaction.commandName);
		...
		if (command.ownersOnly === true) {
			let subs = Broadcast.get(`Broadcast2`)
