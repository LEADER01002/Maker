...
*/const { Events, Interaction, EmbedBuilder ,InteractionType } = require('discord.js');
const { Database } = require("st.db")
const blacklist = require(`../../../tokens/tokens.json`)
const client8 = require(`../blacklist-Bots.js`)
module.exports = {
    ...
    if (interaction.isChatInputCommand()) {
      console.log('tst2')
	    if(interaction.user.bot) return;
	     let client = interaction.client8;
		const command = interaction.client8.blacklistSlashCommands.get(interaction.commandName);
		...
		if (command.ownersOnly === true) {
			let subs = blacklist.get(`blacklist`)
