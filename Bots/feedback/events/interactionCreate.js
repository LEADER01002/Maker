...
*/const { Events, Interaction, EmbedBuilder ,InteractionType } = require('discord.js');
const { Database } = require("st.db")
const feedback = require(`../../../tokens/tokens.json`)
const client11 = require(`../feedback-Bots.js`)
module.exports = {
    ...
    if (interaction.isChatInputCommand()) {
      console.log('tst2')
	    if(interaction.user.bot) return;
	     let client = interaction.client11;
		const command = interaction.client11.feedbackSlashCommands.get(interaction.commandName);
		...
		if (command.ownersOnly === true) {
			let subs = feedback.get(`feedback`)
