...
*/const { Events, Interaction, EmbedBuilder ,InteractionType } = require('discord.js');
const { Database } = require("st.db")
const autoline = require(`../../../tokens/tokens.json`)
const client10 = require(`../autoline-Bots.js`)
module.exports = {
    ...
    if (interaction.isChatInputCommand()) {
      console.log('tst2')
	    if(interaction.user.bot) return;
	     let client = interaction.client10;
		const command = interaction.client10.autolineSlashCommands.get(interaction.commandName);
		...
		if (command.ownersOnly === true) {
			let subs = autoline.get(`autoline`)
