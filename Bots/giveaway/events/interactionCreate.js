...
*/const { Events, Interaction, EmbedBuilder ,InteractionType } = require('discord.js');
const { Database } = require("st.db")
const giveaway = require(`../../../tokens/tokens.json`)
const client14 = require(`../giveaway-Bots.js`)
module.exports = {
    ...
    if (interaction.isChatInputCommand()) {
      console.log('tst2')
	    if(interaction.user.bot) return;
	     let client = interaction.client14;
		const command = interaction.client14.giveawaySlashCommands.get(interaction.commandName);
		...
		if (command.ownersOnly === true) {
			let subs = giveaway.get(`giveaway`)
