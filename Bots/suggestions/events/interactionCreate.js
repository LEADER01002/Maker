...
*/const { Events, Interaction, EmbedBuilder ,InteractionType } = require('discord.js');
const { Database } = require("st.db")
const suggestions = require(`../../../tokens/tokens.json`)
const client12 = require(`../suggestions-Bots.js`)
module.exports = {
    ...
    if (interaction.isChatInputCommand()) {
      console.log('tst2')
	    if(interaction.user.bot) return;
	     let client = interaction.client12;
		const command = interaction.client12.suggestionsSlashCommands.get(interaction.commandName);
		...
		if (command.ownersOnly === true) {
			let subs = suggestions.get(`suggestions`)
			let info = subs.find(a => a.guildid == interaction.guild.id)
			let ownerid = info.owner
